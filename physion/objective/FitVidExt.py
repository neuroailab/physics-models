...
class FitVidModelExt(PytorchModel):

    def get_model(self):

        cfg = self.pretraining_cfg.MODEL
...
def ssim(video_1, video_2, max_val):
    ...
    dist = np.array([structural_similarity(video_1[i], video_2[i], data_range=max_val, channel_axis=2) for i in
                     range(len(video_1))])
...
class ExtractionObjective(FitVidModelExt, ExtractionObjectiveBase):
...
def save_vis(input_frames, pretraining_cfg, output_dir, prefix=0, artifact_path='videos', target_size=128):
    ...
    for i in range(n_vis_per_batch):
        ...
        for lbl, arr in input_frames.items():
            ...
            arr = (255 * torch.permute(arr[i], (0, 2, 3, 1)).numpy()).astype(
                np.uint8)  # (T,C,H,W) => (T,H,W,C), then [0.,1.] => [0, 255]
